...
public abstract class BasePage {
    ...
    public void clickWithJSExecutor(WebElement element, int x, int y){ //метод, позволяющий залезть в текст под рекламой
        //JavascriptExecutor js = (JavascriptExecutor) driver; // эта строка уже вверху
    ...
    public void verifyLinks(String linkUrl) {
        try {
            ...
            //создаем соединение с сервером и получаем статус ОТВЕТА:
            HttpURLConnection connection = (HttpURLConnection) url.openConnection();
            connection.setConnectTimeout(5000);
            connection.connect();
            //получаем статус ответа - статус-код
    ...
    public void hideIframes() { //методы, убирающие лишнее: рекламу, footer
        hideAd(); //метод, скрывающий рекламу
        hideFooter(); //метод , скрывающий footer
    }
